...
int main(int argc, char **argv)
{
    ...
    for(int merge_size = 2; merge_size <= world_size; merge_size *= 2){
        if(world_rank % merge_size == 0){
            int child = world_rank + (merge_size / 2);
            MPI_Recv(&in_circle_num_received, 1, MPI_LONG_LONG_INT, child, MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            in_circle_num += in_circle_num_received;
        } 
        else{
            int parent = world_rank - (merge_size / 2);
            MPI_Send(&in_circle_num, 1, MPI_LONG_LONG_INT, parent, 0, MPI_COMM_WORLD);
            break;
        }
    }
